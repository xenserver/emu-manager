...
struct em_client
{
    char buf[EM_CLIENT_BUF_SIZE];   /* Read buffer */
    int nbytes;                     /* Number of bytes of data in buf */
    int fd;                         /* Socket connection */
    bool needs_return;              /* True if waiting for a "return" reply */
    json_tokener *tok;              /* Used for decoding messages. */
    em_client_event_cb event_cb;    /* Called when events are received */
    void *data;                     /* User data for event_cb */
};
